...
- the customer intent is clear;
- the policy is known;
- the answer is in the knowledge base;
- the risk is low;
- the output can be delivered without changing the system.
...
- the customer's real problem is not the stated problem;
- policy and trust conflict;
- billing identity is unclear;
- the support issue exposes product confusion;
- the customer needs accountability, not just an answer;
- the system needs to be changed so the issue stops recurring.
...
1. What work is stable-frame enough for the agent?
2. Where does the frame break?
